...
- 参数寄存器：`%rdi` `%rsi` `%rdx` `%r10` `%r8` `%r9`<br><span class="text-sm text-gray-5">（注意和过程调用有出入，第 4 个参数是 `%r10`，而不是过程调用中的 `%rcx`）</span>
...
![syscall](./res/image/slides.assets/syscall.png){.mx-auto.h-30}
...
root           1       0  0 9月12 ?       00:02:44 /sbin/init
root           2       0  0 9月12 ?       00:00:00 [kthreadd]
root           3       2  0 9月12 ?       00:00:00 [pool_workqueue_release]
root           4       2  0 9月12 ?       00:00:00 [kworker/R-rcu_g]
```
...
例2：在服务器上后台运行（`nohup xxx &`）的一些程序。
```bash
UID          PID    PPID  C STIME TTY          TIME CMD
root     1733599       1  0 10月23 ?      00:04:29 python qcapp.py
root     1773827       1  0 10月25 ?      00:02:20 /usr/lib/snapd/snapd
root     1883468       1  0 10月27 ?      00:00:01 python app.py
root     1896893       1  0 10月28 ?      01:35:06 python webui.py
root     2663552       2  0 11月07 ?      00:00:23 [kworker/3:2-mm_percpu_wq]
root     2672462 1883468  0 11月07 ?      00:19:45 /home/node/anaconda3/bin/python app.py
root     2685708 2685682  0 11月07 ?      00:01:20 python /code/app_local.py
root     2685815 2685708  0 11月07 ?      00:07:07 python /code/app_local.py
root     2685816 2685708  0 11月07 ?      00:06:56 python /code/app_local.py
root     2685824 2685708  0 11月07 ?      00:31:24 python /code/app_local.py
root     2763412       1  0 11月09 ?      00:08:28 redis-server *:6379
root     2763552       1  0 11月09 ?      00:14:37 python app.py
```
...
拓扑排序：不违逆这张图中的箭头方向。(就是离散中的那个拓扑排序)
...
- 如果 `pid > 0`，则将信号 `sig` 发送给<b>进程 ID </b>为 `pid` 的进程
- 如果 `pid == 0`，则将信号 `sig` 发送给与调用 `kill` 的进程<b>属于同一个进程组的所有进程</b>
- 如果 `pid < 0`，则将信号 `sig` 发送进程组 `|pid|` 内的<b>所有进程</b>
...
问题：
-  `Sigprocmask(SIG_BLOCK, &mask_all, &prev_all);`之前子进程就已经终止
- 父进程先执行delete，然后才add
...
- 父进程从开始fork到addjob结束之前都不会处理`SIGCHLD`信号
- 无论子进程何时结束，父进程一定是先add，然后delete。
...
// 定义一个处理 SIGCHLD 信号的处理函数
{
    int olderrno = errno; // 保存当前的 errno 值
    pid = waitpid(-1, NULL, 0); 
    // 等待任意子进程结束，并将其 pid 保存到全局变量 pid 中
...
- sigsuspend暂时取消对`SIGCHLD`的阻塞，返回前恢复。
- 如果收到的是`SIGCHLD`那么跳出循环
...
- `setjmp`保存当前调用环境，返回值不能赋值给变量，但能用在switch和条件语句测试中
- `setjmp`多次返回,`longjmp`从不返回
- `longjmp`从深层嵌套中立即返回（从最近一次初始化env的`setjmp`返回）
- 通过switch `setjmp`的返回值判断是第一次调用的返回还是`longjmp`引起的返回，并且可区分错误原因
...
- 作用为初始化后每次收到`SIGINT`后返回到`if`处，返回值为1，会输出“resting”然后再次进入无限循环
- 必须先`sigsetjmp`再设置信号处理程序，否则可能在之间收到信号，进而先调用`siglongjmp`
- `sigsetjmp`和`siglongjmp`不是信号安全函数，但只要保证程序不会跳转而后执行非信号安全函数就没有问题
...
![image-20241017175428523](./newres/image/W11/1.png)
...
![image-20241017175428523](./newres/image/W11/2.png){.w-100}
</div>
<div>

![image-20241017175428523](./newres/image/W11/3.png){.w-180}
...
int main()
{
    ...
    if(pid == 0)
    {
        printf("child:PKUICS25fall!\n");
    }
    else
    {
        sleep(5);
        printf("main:PKUICS25fall!\n");
        sleep(2);
...
- 示例中pid：2867235对应的`app.py`为一个用flask搭建的网页程序，并且会调用其他程序进行网络通信、视频编辑、语音识别等功能，其中会调用显卡，并且在名为`Short`的conda虚拟环境中运行。
- 实例中pid： 103298/103706为左侧代码gcc编译后两次运行时父进程的pid。
...
- STRACE：打印正在运行程序和它的子进程调用的每个系统调用的轨迹。
- `strace -p 2867235`

![image-20241017175428523](./newres/image/W11/4.png){.w-160}
...
![image-20241017175428523](./newres/image/W11/8.png){.w-160}
...
![image-20241017175428523](./newres/image/W11/5.png){.w-130}
...
![image-20241017175428523](./newres/image/W11/9.png){.w-130}
...
![image-20241017180307201](./newres/image/W11/10.png)

---

# E9

![image-20241017180307201](./newres/image/W11/10.png){.w-170}
![image-20241017180307201](./newres/image/W11/11.png)
