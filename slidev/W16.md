...
title: W16-Concurrent Programming
info: |
...
![cp](./newres/image/W16/12-Concurrent-Programming/cp.png)
...
![conc_based_on_processes](./newres/image/W16/12-Concurrent-Programming/conc_based_on_processes.png){.mx-auto.h-50}
...
![conc_based_on_processes_logic_flow](./newres/image/W16/12-Concurrent-Programming/conc_based_on_processes_logic_flow.png){.mx-auto.h-100}
...
![thread-stack](./newres/image/W16/12-Concurrent-Programming/thread-stack.png){.h-100}

</div>

<div>

![thread-concurrent](./newres/image/W16/12-Concurrent-Programming/thread-concurrent.png)
...
- **对于主线程，`pthread_exit`会等待其他线程终止后再终止主线程和整个进程**{.text-sky-5}
- **但是`exit`不会等待对等线程，直接终止进程**{.text-sky-5}
...
```c{16,18,22,23,27,28}{maxHeight: '400px',lines: true}
#include "csapp.h"
...
```c{16,18,22,23,27,28,31}{maxHeight: '400px',lines: true}
#include "csapp.h"
...
![thread-parallel](./newres/image/W16/12-Concurrent-Programming/thread-parallel.png){.h-60.m-auto}
...
![1](./newres/image/W16/1.png)
</div>
<div>

![1](./newres/image/W16/2.png)
</div>
</div>
...
- 只有向右和向上的轨迹是合法的
- 操作共享变量的操作构成了一个临界区
- 每个线程在执行它的临界区中指令时，拥有对共享变量互斥的访问
- 两个临界区的交集形成的状态空间称为不安全区，**不包括交界的状态**
- 安全轨迹线与不安全轨迹线

</div>
<div>

![1](./newres/image/W16/3.png){.w-60}
![1](./newres/image/W16/4.png){.w-67}
</div>
</div>
...
- 写者优先。在一个写者后到达的读者必须等待，即使这个写者也在等待。
...
- 参考书中代码。主要注意`Pthread_once`和`pthread_once_t`的用法
...
- 可以通过给线程传递一个唯一的ID指明要进行并行的哪一部分
- 减少P/V的同步次数以降低开销，例如先存在局部变量统一写（类似于cache部分）
- 当线程数大于核数时，因为上下文切换开销，运行时间可能会变长
- 加速比（强扩展）：$S_p = \frac{T_1}{T_p}$，$p$为核数，$T_k$为$k$个核上运行时间
- 绝对加速比：$T_1$为程序顺序版本运行时间
- 相对加速比：$T_1$为程序并行版本在一个核上运行时间
- 相对加速比因同步开销会较大，但绝对加速比更难测量（需要额外编写顺序版本）
- 效率：$E_p = \frac{S_p}{p} = \frac{T_1}{pT_p}$，并行化造成开销的衡量
- 弱扩展：增加处理器数量的同时，增加问题规模。此时加速比和效率被表达为单位时间完成的工作总量
...
![thread_safe_func](./newres/image/W16/12-Concurrent-Programming/thread_safe_func.png)
...
![6](./newres/image/W16/6.png){.w-100}
...
![multithread](./newres/image/W16/12-Concurrent-Programming/multithread.png){.h-80.mx-auto}
...
- 在主线程对`i`加一和对等线程对`myid`赋值之间有竞争，取决于对线程的调度情况
...
![6](./newres/image/W16/8.jpg){.h-100.mx-auto}
...
![6](./newres/image/W16/7.png){.w-100}
</div>

<div>

- 两个信号量的禁止区域重叠，可能导致死锁区域
- 难以复现，因取决于调度
- 对于二元信号量可以有避免死锁的规则
- 给定所有互斥信号的一个全序，如果每个线程都是以同样的顺序**获得它的互斥锁**，那么这个程序就是无死锁的。
- 与书上区别：**不用看释放顺序**（勘误）
...
![9](./newres/image/W16/8.png){.h-80.mx-auto}

- 主进程没有等待对等线程终止就直接调用`exit(0)`退出，对等线程还没来得及输出就被杀死了。
...
![9](./newres/image/W16/9.png){.w-120}

![9](./newres/image/W16/4.png){.w-120}
...
![9](./newres/image/W16/13.png){.w-75}
...
![9](./newres/image/W16/14.png){.w-68}
...
![9](./newres/image/W16/12.png){.w-150}

- A：1同时占用ab、ac；2同时占用bc；3同时占用ab
- B：2和3
- C：每个线程都按同样的顺序加锁（例如abc）
...
<div>
  <text class="text-17 font-bold gradient-text">期末考试时间</text>
</div>

</br></br>

# 写清大班号和小班号！{.text-gradient.font-bold}
...
<div>
  <text class="text-17 font-bold gradient-text">期末顺利&万事顺遂</text>
</div>

</br></br>

# 今天的问题，明天就有回答{.text-gradient.font-bold}
