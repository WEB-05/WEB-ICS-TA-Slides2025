...
![physical_addressing](./newres/image/W13/09-Virtual-Memory/physical_addressing.png){.h-55.mx-auto}
...
![virtual_addressing](./newres/image/W13/09-Virtual-Memory/virtual_addressing.png){.h-55.mx-auto}
...
![memory_hierarchy](./newres/image/W13/09-Virtual-Memory/memory_hierarchy.png){.mx-auto}
...
![vm_as_cache](./newres/image/W13/09-Virtual-Memory/vm_as_cache.png){.mx-auto}
...
![page_table](./newres/image/W13/09-Virtual-Memory/page_table.png){.h-60.mx-auto}
...
![page_table](./newres/image/W13/09-Virtual-Memory/page_table.png){.h-70.mx-auto}
...
![review_fully_associative](./newres/image/W13/09-Virtual-Memory/review_fully_associative.png){.mx-auto}
...
![private_address_space](./newres/image/W13/09-Virtual-Memory/private_address_space.png){mx-auto}

</div>
<div :class="$clicks==2 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute>

![page_table](./newres/image/W13/09-Virtual-Memory/page_table.png){mx-auto}

</div>
<div :class="$clicks>=3 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute>

![vm_share_page](./newres/image/W13/09-Virtual-Memory/vm_share_page.png){mx-auto}
...
![mem_protection](./newres/image/W13/09-Virtual-Memory/mem_protection.png){.h-70.mx-auto}
...
![vm_translation](./newres/image/W13/09-Virtual-Memory/vm_translation.png){.mx-auto}
...
![vm_translation_detail_hit](./newres/image/W13/09-Virtual-Memory/vm_translation_detail_hit.png){.mx-auto}
...
![vm_translation_detail_miss](./newres/image/W13/09-Virtual-Memory/vm_translation_detail_miss.png){.mx-auto}
...
![page_table_cache](./newres/image/W13/09-Virtual-Memory/page_table_cache.png){.mx-auto.h-70}
...
![tlb](./newres/image/W13/09-Virtual-Memory/tlb.png){.mx-auto}
![vp_tlb](./newres/image/W13/09-Virtual-Memory/vp_tlb.png){.mx-auto}
...
![vp_tlb](./newres/image/W13/09-Virtual-Memory/vp_tlb.png){.mx-auto}
![cache_address](./newres/image/W13/09-Virtual-Memory/cache_address.png){.mx-auto}
...
![multi_level_page_table](./newres/image/W13/09-Virtual-Memory/multi_level_page_table.png){.mx-auto}


</div>
<div :class="$clicks==1 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute>

![multi_level_page_table_2](./newres/image/W13/09-Virtual-Memory/multi_level_page_table_2.png){.mx-auto}
...
![multi_level_page_table](./newres/image/W13/09-Virtual-Memory/multi_level_page_table.png){.mx-auto.h-50}


![multi_level_page_table_2](./newres/image/W13/09-Virtual-Memory/multi_level_page_table_2.png){.mx-auto.h-50}
...
![address_translation_all](./newres/image/W13/09-Virtual-Memory/address_translation_all.png){.mx-auto}
...
![i7_p1_to_p3](./newres/image/W13/09-Virtual-Memory/i7_p1_to_p3.png){.mx-auto.h-100}

</div>
<div :class="$clicks==1 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute w-full>

![i7_p4](./newres/image/W13/09-Virtual-Memory/i7_p4.png){.mx-auto.h-100}
...
![linux_vm_process](./newres/image/W13/1.png){.mx-auto.h-100}
...
![linux_vm_process](./newres/image/W13/09-Virtual-Memory/linux_vm_process.png){.mx-auto.h-100}
...
![linux_vm_organization](./newres/image/W13/09-Virtual-Memory/linux_vm_organization.png){.mx-auto.h-90}
...
![page_fault](./newres/image/W13/09-Virtual-Memory/page_fault.png){.mx-auto.h-90}
...
![shared_object](./newres/image/W13/09-Virtual-Memory/shared_object.png){.mx-auto}
...
![cow](./newres/image/W13/09-Virtual-Memory/cow.png){.mx-auto}
...
![loader](./newres/image/W13/09-Virtual-Memory/loader.png){.mx-auto.h-100}
...
![mmap](./newres/image/W13/09-Virtual-Memory/mmap.png){.mx-auto}
...
<br/>基本上就是堆的相关内容，大家做 malloclab 一定会狠狠体验到的。
...
![PixPin_2024-11-26_16-06-14](./newres/image/W13/2.png){.w-150}
![PixPin_2024-11-26_16-06-14](./newres/image/W13/3.png){.w-150}

</div>
<div>

![PixPin_2024-11-26_16-06-14](./newres/image/W13/4.png){.w-150}
...
![PixPin_2024-11-26_16-06-14](./newres/image/W13/5.png){.w-100}

---

# 9.12

![PixPin_2024-11-26_16-06-14](./newres/image/W13/6.png){.w-100}

---

# 9.13

![PixPin_2024-11-26_16-06-14](./newres/image/W13/7.png){.w-100}
...
```c
    ...
    while (fgets(buf, BUF_SIZE, maps_file) != NULL) {
        // 解析 VMA 的起始地址、结束地址、权限（忽略后续无关字段）
        if (sscanf(buf, "%llx-%llx %4s", &vma_start, 
          &vma_end, perms) != 3) {
            continue;  // 解析失败则跳过该行
        }
        // 4. 筛选：仅保留有访问权限的 VMA（r/w/x 至少一个）
        if (strchr(perms, 'r') == NULL &&
            strchr(perms, 'w') == NULL &&
            strchr(perms, 'x') == NULL) {
            continue;
        }
        // 5. 筛选：仅保留用户态地址（区分 32/64 位系统）
        if (sizeof(void*) == 8) {  // 64位系统
            if (vma_start >= 0x800000000000ULL) {
              continue;}
        } else {  // 32位系统
            if (vma_start >= 0xC0000000UL) {
                continue;}}
...
```c
// 6. 计算该 VMA 内的所有虚拟页号并打印
        for (unsigned long long addr = vma_start;
           addr < vma_end; addr += page_size) {
        unsigned long long page_num = addr / page_size;
            printf("%llu with %s\n", page_num, perms);
        }
    }
    // 检查读取过程是否出错
